function avi_filename = show_demixed_patch_vid(obj,save_avi, kt, frame_range, amp_ac, range_ac, range_Y, multi_factor, use_craw)
% function [ImY, ImYbg, ImRawBG, ImDenoised, ImResid, ImDemixed] =  show_demixed_video1(obj,save_avi, kt, frame_range, amp_ac, range_ac, range_Y, multi_factor, use_craw)
% function ImDemixed =  show_demixed_video1(obj,save_avi, kt, frame_range, amp_ac, range_ac, range_Y, multi_factor, use_craw)
...
%% play and save
ax_y =   axes('position', [0.05, 0.25, .45, .4]);
...
ax_mix =    axes('position', [0.55, 0.25, .4, 0.4]);
tt0 = (t_begin-1);
...
%     axes(ax_bg); cla;
%     ImYbg = imagesc(Ybg(:, :, m),range_Y);
%     %     set(gca, 'children', flipud(get(gca, 'children')));
%     axis equal off tight;
%     title('Background');
%     
%     axes(ax_signal); cla;
%     ImRawBG = imagesc(double(Y(:, :, m))-Ybg(:, :, m), range_ac); hold on;
%     %     set(gca, 'children', flipud(get(gca, 'children')));
...
%     ImDenoised = imagesc(img_ac, range_ac);
%     %     imagesc(Ybg(:, :, m), [-50, 50]);
%     title(sprintf('Denoised X %d', multi_factor));
%     axis equal off tight;
%     
%     axes(ax_res); cla;
%     ImResid = imagesc(double(Y(:, :, m))-Ybg(:, :, m)-img_ac, range_res);
%     %     set(gca, 'children', flipud(get(gca, 'children')));
...
%     ImDemixed = imagesc(Y_mixed(:, m,:));  hold on;
    title('Demixed');
%     text(1, 10, sprintf(' %.2f ', (tt)/obj.Fs), 'color', 'w', 'fontweight', 'bold');
%     
