...
%% choose data
neuron = Sources2D();
nam = get_fullname('./msCam9.avi');          % this demo data is very small, here we just use it as an example
...
avi_filename = neuron.show_demixed_video1(save_demixed, kt, [], amp_ac, range_ac, range_Y, multi_factor);
% avi_filename = neuron.show_demixed_video1(save_demixed, kt);


% neuron.show_demixed_video1(save_demixed, kt, [], amp_ac, range_ac, range_Y, multi_factor);
% Resid = neuron.show_demixed_video1(save_demixed, kt, [], amp_ac, range_ac, range_Y, multi_factor);       %% compute the residual
...
im = struct;
% im = neuron.reshape(neuron.A(:,3),2);
imAll = neuron.reshape(full(neuron.A),2);
Totalimage = sum(imAll,3);
imagesc(Totalimage);

% for i = 1:15
% 
%     imstack = neuron.image(neuron.A(:,i));
% i = i+1;
% end
